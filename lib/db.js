...
const BACKUP_TABLES = Object.freeze(['settings','users','persons','branches','comments','audit_logs','page_visits']);
const MAX_BACKUP_UPLOAD_BYTES = 140 * 1024 * 1024;
const MAX_BACKUP_ROWS = 500000;
const UPLOAD_DIR = path.join(DATA_DIR, 'uploads');
const UPLOAD_LAYOUT = Object.freeze({ logo:'Logo', qrcode:'qrcode', profiles:'profiles', gallery:'gallery', contacts:'contacts', temple:'temple', legacy:'_legacy' });

function validUploadName(name) { return /^[a-f0-9-]+\.(?:png|jpg|jpeg|webp)$/i.test(String(name || '')); }
function normalizeUploadPath(value) {
  const raw=String(value||'').replace(/\\/g,'/').replace(/^\/+|\/+$/g,'');
  if(!raw || raw.includes('..') || raw.includes('\0')) return '';
  const parts=raw.split('/');
  if(parts.some((part)=>!part || !/^[A-Za-z0-9._-]+$/.test(part))) return '';
...
function normalizeRichTextContent(value, maxChars = 8000) {
  ...
  for (const item of parsed.slice(0, 600)) {
    if (!item || typeof item !== 'object' || total >= maxChars) continue;
    ...
    const token = { text, bold:!!item.bold, italic:!!item.italic, underline:!!item.underline, strike:!!item.strike, size, color, font, align };
    const prev = out[out.length - 1];
    if (prev && prev.bold===token.bold && prev.italic===token.italic && prev.underline===token.underline && prev.strike===token.strike && prev.size===token.size && prev.color===token.color && prev.font===token.font && prev.align===token.align) prev.text += text;
    else out.push(token);
  }
...
class Store {
  ...
  ensureUploadLayout() {
    fs.mkdirSync(UPLOAD_DIR,{recursive:true});
    for(const name of [UPLOAD_LAYOUT.logo,UPLOAD_LAYOUT.qrcode,UPLOAD_LAYOUT.profiles,UPLOAD_LAYOUT.gallery,UPLOAD_LAYOUT.contacts,UPLOAD_LAYOUT.temple]) fs.mkdirSync(path.join(UPLOAD_DIR,name),{recursive:true});
  }
  ...
  galleryAlbumUploadDir(albumOrId) {
    ...
    if(!album)return '';
    const folder=String(album.storage_folder||'').replace(/[^A-Za-z0-9._-]/g,'');
    if(!folder)return '';
    const rel=`${UPLOAD_LAYOUT.gallery}/${folder}`; fs.mkdirSync(path.join(UPLOAD_DIR,...rel.split('/')),{recursive:true}); return rel;
  }
  migrateUploadLayout() {
    this.ensureUploadLayout();
    ...
    for(const album of albums){
      if(album.storage_folder && /^[A-Za-z0-9._-]+$/.test(album.storage_folder)){ fs.mkdirSync(path.join(UPLOAD_DIR,UPLOAD_LAYOUT.gallery,album.storage_folder),{recursive:true}); continue; }
  ...
  updateSettings(input, actorId) {
    ...
    const normalizeSetting=(key,value)=>{
      ...
      if(['tree_subtitle_content','tree_footer_content','gallery_intro_content','gallery_footer_content','footer_author_content','contact_intro_content','contact_footer_content','contact_map_address_content'].includes(key)) return normalizeRichTextContent(raw, 8000);
      ...
      if(key==='public_comments_enabled'||key==='fund_support_enabled') return raw==='0'?'0':'1';
    ...
    this.audit(actorId, 'settings.update', 'settings', null, JSON.stringify(Object.fromEntries(entries)));
  ...
  rawPeople() {
    return this.db.prepare('SELECT * FROM persons ORDER BY level,birth_order,sort_order,full_name').all().map(this.rowToPerson);
  }
  rowToPerson(row) {
    if (!row) return null;
    return {
  ...
  presentPerson(person, publicOnly) {
    ...
    if (p.privacy_mode === 'private') {
      return {
        ...
        father_id:p.father_id, mother_id:p.mother_id, spouse_ids:p.spouse_ids, divorced_spouse_ids:p.divorced_spouse_ids, step_parent_ids:p.step_parent_ids,
        sibling_ids:[], birth_order:p.birth_order, is_adopted:p.is_adopted, is_deceased:p.is_deceased, age_years:p.age_years, privacy_mode:'private',
      };
    }
  ...
  validateRelations(personId, input) {
    ...
    const arrays = ['spouse_ids','divorced_spouse_ids','step_parent_ids','sibling_ids'];
  ...
  normalizePersonInput(input, current = {}) {
    ...
    return {
      ...
      spouse_ids: jsonArray(input.spouse_ids ?? current.spouse_ids), divorced_spouse_ids: jsonArray(input.divorced_spouse_ids ?? current.divorced_spouse_ids),
      ...
      birth_order: Math.min(50,Math.max(1,Number(input.birth_order ?? current.birth_order ?? 1)||1)),
  ...
  updatePerson(id, input, actorId) {
    ...
    this.db.prepare(`UPDATE persons SET family_code=?,full_name=?,gender=?,birth_date=?,birth_place=?,death_date=?,death_place=?,is_deceased=?,level=?,father_id=?,mother_id=?,spouse_ids=?,divorced_spouse_ids=?,step_parent_ids=?,sibling_ids=?,birth_order=?,is_adopted=?,occupation=?,details=?,source_citations=?,image_path=?,privacy_mode=?,sort_order=?,updated_by=?,updated_at=? WHERE id=?`)
  ...
  ensureReciprocalSpouses(id, spouseIds, divorcedIds, actorId) {
    const all=this.rawPeople(); const byId=new Map(all.map((p)=>[p.id,p]));
    this.withTransaction(()=>{
      for (const p of all) {
        if (p.id===id) continue;
        const should = spouseIds.includes(p.id);
        let spouses=jsonArray(p.spouse_ids); let divorced=jsonArray(p.divorced_spouse_ids); let changed=false;
        if (should && !spouses.includes(id)) { spouses.push(id); changed=true; }
        if (!should && spouses.includes(id)) { spouses=spouses.filter((x)=>x!==id); divorced=divorced.filter((x)=>x!==id); changed=true; }
        ...
        if(changed) this.db.prepare('UPDATE persons SET spouse_ids=?,divorced_spouse_ids=?,updated_by=?,updated_at=? WHERE id=?').run(JSON.stringify(spouses),JSON.stringify(divorced),actorId,nowIso(),p.id);
      }
    });
  }
  deletePerson(id, actorId) {
    ...
    this.withTransaction(()=>{
      for(const p of all){ if(p.id===id) continue;
        ...
        this.db.prepare(`UPDATE persons SET father_id=CASE WHEN father_id=? THEN NULL ELSE father_id END,mother_id=CASE WHEN mother_id=? THEN NULL ELSE mother_id END,spouse_ids=?,divorced_spouse_ids=?,step_parent_ids=?,sibling_ids=?,updated_by=?,updated_at=? WHERE id=?`)
          .run(id,id,JSON.stringify(spouse),JSON.stringify(div),JSON.stringify(step),JSON.stringify(sib),actorId,nowIso(),p.id);
      }
  ...
  stats(){ const all=this.rawPeople(); const base=this.treeStats(all); return { ...base, generations:all.length?Math.max(...all.map((p)=>p.level)):0, branches:Number(this.db.prepare('SELECT COUNT(*) AS c FROM branches').get().c||0), comments:Number(this.db.prepare('SELECT COUNT(*) AS c FROM comments WHERE deleted_at IS NULL').get().c||0), users:Number(this.db.prepare('SELECT COUNT(*) AS c FROM users WHERE is_active=1').get().c||0) }; }
  ...
  restoreDataDirectory(stagedDataDir, actorId, currentSessionHash = null) {
    ...
    try {
      ...
      try { this.audit(sessionPreserved ? actorId : null, 'backup.restore', 'backup', null, JSON.stringify({ mode:'data-folder', ...summary })); } catch {}
      fs.rmSync(oldDir, { recursive: true, force: true }); oldMoved = false;
      const uploadsDir = path.join(DATA_DIR, 'uploads');
      const uploads = countFilesRecursive(uploadsDir);
      return { ok:true, session_preserved:sessionPreserved, summary:{ ...summary, uploads } };
    } catch (error) {
  ...
  exportFullBackup(){
    ...
    return {
      ...
      summary:{people:tables.persons.length,branches:tables.branches.length,users:tables.users.length,comments:tables.comments.length,audit_logs:tables.audit_logs.length,page_visits:tables.page_visits.length,uploads:uploads.length,upload_bytes:uploadBytes},
  ...
  restoreFullBackup(payload, actorId, currentSessionHash=null){
    ...
    if(!/^[a-f0-9]{64}$/i.test(expectedTablesHash) || !safeHashEqual(expectedTablesHash,actualTablesHash)) throw new Error('Dữ liệu bảng không vượt qua kiểm tra toàn vẹn của bản sao lưu.');
    ...
    try{
      ...
      this.db.exec('DELETE FROM branches; DELETE FROM comments; DELETE FROM audit_logs; DELETE FROM page_visits; DELETE FROM persons; DELETE FROM users; DELETE FROM settings;');
      insertRows('settings',tables.settings); insertRows('users',tables.users); insertRows('persons',tables.persons); insertRows('branches',tables.branches); insertRows('comments',tables.comments); insertRows('audit_logs',tables.audit_logs); insertRows('page_visits',tables.page_visits);
    ...
    return {ok:true,session_preserved:sessionPreserved,exported_at:payload.exported_at||null,summary:{people:tables.persons.length,branches:tables.branches.length,users:tables.users.length,comments:tables.comments.length,uploads:decoded.length,page_visits:tables.page_visits.length}};
  }
...
module.exports = { Store, DATA_DIR, DB_PATH, UPLOAD_DIR, UPLOAD_LAYOUT, nowIso, uuid, jsonArray, normalizeUploadPath, uploadUrl, uploadFullPath, countFilesRecursive };
