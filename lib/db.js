...
const NEWS_ALLOWED_TAGS = new Set(['p','div','br','h2','h3','h4','blockquote','ul','ol','li','strong','b','em','i','u','s','strike','span','a','img','figure','figcaption','hr']);
...
function normalizeNewsStyle(style,tag){
  ...
  for(const chunk of String(style||'').split(';')){
    ...
    if(key==='text-align' && ['left','center','right','justify'].includes(value.toLowerCase()) && ['p','div','h2','h3','h4','blockquote','figcaption'].includes(tag)) allowed.push(`text-align:${value.toLowerCase()}`);
    else if(key==='color' && (/^#[0-9a-f]{6}$/i.test(value)||/^rgb\(\s*\d{1,3}\s*,\s*\d{1,3}\s*,\s*\d{1,3}\s*\)$/i.test(value))) allowed.push(`color:${value}`);
    else if(key==='font-size') { const m=value.match(/^(\d{1,2})(?:px)?$/i); if(m){const n=Math.max(10,Math.min(56,Number(m[1])));allowed.push(`font-size:${n}px`);} }
    else if(key==='font-weight' && /^(?:bold|[5-9]00)$/i.test(value)) allowed.push('font-weight:700');
    else if(key==='font-style' && value.toLowerCase()==='italic') allowed.push('font-style:italic');
    else if(key==='text-decoration') { const v=value.toLowerCase(); const parts=[]; if(v.includes('underline'))parts.push('underline'); if(v.includes('line-through'))parts.push('line-through'); if(parts.length)allowed.push(`text-decoration:${parts.join(' ')}`); }
