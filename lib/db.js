...
class Store {
  ...
  normalizeContributionInput(input,current={}) {
    ...
    return {donor_name:donorName,address,contribution_content:contributionContent,amount,contribution_date:contributionDate,notes};
  }
  ...
  topContributors(limit=10){
    ...
    return this.db.prepare(`SELECT MIN(id) AS id, donor_name, COUNT(*) AS contribution_count, SUM(amount) AS amount, MAX(contribution_date) AS contribution_date FROM contributions GROUP BY donor_name COLLATE NOCASE ORDER BY amount DESC, contribution_date DESC, donor_name COLLATE NOCASE LIMIT ?`).all(n).map((r)=>{
      const latest=this.db.prepare("SELECT address FROM contributions WHERE donor_name=? COLLATE NOCASE AND trim(address)<>'' ORDER BY contribution_date DESC,updated_at DESC LIMIT 1").get(r.donor_name);
      return {id:r.id,donor_name:r.donor_name,address:latest?.address||'',contribution_content:`Tổng ${Number(r.contribution_count||0)} lần công đức`,amount:Number(r.amount||0),contribution_date:r.contribution_date,notes:`Tổng hợp từ ${Number(r.contribution_count||0)} lần công đức`,contribution_count:Number(r.contribution_count||0)};
    });
  }
  ...
  createContribution(input,actorId){const x=this.normalizeContributionInput(input);const id=uuid(),t=nowIso();this.db.prepare('INSERT INTO contributions(id,donor_name,address,contribution_content,amount,contribution_date,notes,created_by,updated_by,created_at,updated_at) VALUES (?,?,?,?,?,?,?,?,?,?,?)').run(id,x.donor_name,x.address,x.contribution_content,x.amount,x.contribution_date,x.notes,actorId,actorId,t,t);this.audit(actorId,'contribution.create','contribution',id,JSON.stringify({donor_name:x.donor_name,amount:x.amount,date:x.contribution_date}));return this.getContribution(id);}
