...
const BACKUP_TABLES = Object.freeze(['settings','users','persons','branches','contributions','news','comments','audit_logs','page_visits']);
const MAX_BACKUP_UPLOAD_BYTES = 140 * 1024 * 1024;
const MAX_BACKUP_ROWS = 500000;
const UPLOAD_DIR = path.join(DATA_DIR, 'uploads');
const UPLOAD_LAYOUT = Object.freeze({ logo:'Logo', qrcode:'qrcode', profiles:'profiles', gallery:'gallery', contacts:'contacts', temple:'temple', richtext:'richtext', news:'news', legacy:'_legacy' });
...
const RICH_TEXT_FONTS = new Set(['system','segoe','arial','tahoma','verdana','trebuchet','calibri','candara','corbel','helvetica','roboto','noto_sans','dejavu_sans','liberation_sans','times','cambria','palatino','noto_serif','dejavu_serif','liberation_serif']);
const RICH_TEXT_ALIGNS = new Set(['left','center','right','justify']);
function normalizeRichImageWidth(value){ const n=Math.trunc(Number(value)||100); return n>=5&&n<=100&&n%5===0?n:100; }
function normalizeRichTextContent(value, maxChars = 8000) {
  ...
  for (const item of parsed.slice(0, 800)) {
    ...
    if (item.type === 'html') {
      ...
      const html = sanitizeRichHtml(String(item.html||''), Math.max(1,maxChars-total));
      if (!html) continue;
      const plain = newsPlainText(html, Math.max(1,maxChars-total));
      total += plain.length;
    ...
    if (item.type === 'image') {
      ...
      out.push({ type:'image', image_path:imagePath, alt:String(item.alt||'').replace(/[\u0000-\u001f\u007f]/g,' ').trim().slice(0,240), width:normalizeRichImageWidth(item.width), align:RICH_TEXT_ALIGNS.has(String(item.align||'center'))?String(item.align||'center'):'center' });
      continue;
    }
    if (total >= maxChars) continue;
...
const NEWS_ALLOWED_TAGS = new Set(['p','div','br','h2','h3','h4','blockquote','ul','ol','li','strong','b','em','i','u','s','strike','span','a','img','figure','figcaption','hr','table','thead','tbody','tfoot','tr','th','td','caption']);
const NEWS_VOID_TAGS = new Set(['br','img','hr']);
function escapeHtmlText(value){return String(value||'').replace(/&/g,'&amp;').replace(/</g,'&lt;').replace(/>/g,'&gt;');}
function escapeHtmlAttr(value){return escapeHtmlText(value).replace(/"/g,'&quot;').replace(/'/g,'&#39;');}
function decodeHtmlEntityText(value){return String(value||'').replace(/&nbsp;/gi,' ').replace(/&amp;/gi,'&').replace(/&lt;/gi,'<').replace(/&gt;/gi,'>').replace(/&quot;/gi,'"').replace(/&#39;|&#x27;/gi,"'").replace(/&#(\d+);/g,(_,n)=>String.fromCodePoint(Math.min(0x10ffff,Number(n)||32))).replace(/&#x([0-9a-f]+);/gi,(_,n)=>String.fromCodePoint(Math.min(0x10ffff,parseInt(n,16)||32)));}
function newsPlainText(html,max=500){return decodeHtmlEntityText(String(html||'').replace(/<br\s*\/?\s*>/gi,'\n').replace(/<\/(?:p|div|h2|h3|h4|blockquote|li)>/gi,'\n').replace(/<[^>]*>/g,' ')).replace(/[ \t]+/g,' ').replace(/\n\s*/g,'\n').replace(/\n{3,}/g,'\n\n').trim().slice(0,max);}
function normalizeNewsStyle(style,tag){
  const allowed=[];
  for(const chunk of String(style||'').split(';')){
    const [rawKey,...rest]=chunk.split(':'); if(!rawKey||!rest.length)continue;
    const key=rawKey.trim().toLowerCase(), value=rest.join(':').trim(), lower=value.toLowerCase();
    if(key==='text-align' && ['left','center','right','justify'].includes(lower) && ['p','div','h2','h3','h4','blockquote','figcaption','figure','table','th','td','caption'].includes(tag)) allowed.push(`text-align:${lower}`);
    else if(key==='color' && (/^#[0-9a-f]{6}$/i.test(value)||/^rgb\(\s*\d{1,3}\s*,\s*\d{1,3}\s*,\s*\d{1,3}\s*\)$/i.test(value))) allowed.push(`color:${value}`);
    else if(key==='background-color' && ['th','td'].includes(tag) && (/^#[0-9a-f]{6}$/i.test(value)||/^rgba?\(\s*\d{1,3}\s*,\s*\d{1,3}\s*,\s*\d{1,3}(?:\s*,\s*(?:0(?:\.\d+)?|1(?:\.0+)?))?\s*\)$/i.test(value))) allowed.push(`background-color:${value}`);
    else if(key==='border-color' && ['th','td'].includes(tag) && (/^#[0-9a-f]{6}$/i.test(value)||/^rgba?\(\s*\d{1,3}\s*,\s*\d{1,3}\s*,\s*\d{1,3}(?:\s*,\s*(?:0(?:\.\d+)?|1(?:\.0+)?))?\s*\)$/i.test(value))) allowed.push(`border-color:${value}`);
    else if(key==='border-width' && ['th','td'].includes(tag)){const m=value.match(/^(\d)(?:px)?$/i);if(m)allowed.push(`border-width:${Math.max(0,Math.min(8,Number(m[1])))}px`);}
    else if(key==='border-style' && ['th','td'].includes(tag) && ['none','solid','dashed','dotted','double'].includes(lower)) allowed.push(`border-style:${lower}`);
    else if(key==='vertical-align' && ['th','td'].includes(tag) && ['top','middle','bottom'].includes(lower)) allowed.push(`vertical-align:${lower}`);
    else if(key==='padding' && ['th','td'].includes(tag)){const m=value.match(/^(\d{1,2})(?:px)?$/i);if(m)allowed.push(`padding:${Math.max(0,Math.min(30,Number(m[1])))}px`);}
    else if((key==='margin-left'||key==='margin-right') && tag==='table' && (lower==='auto'||lower==='0'||lower==='0px')) allowed.push(`${key}:${lower==='0'?'0px':lower}`);
    else if(key==='width' && ['img','table'].includes(tag)){ const m=value.match(/^(\d{1,3})%$/); if(m){const n=Math.max(5,Math.min(100,Number(m[1])));const snapped=Math.max(5,Math.min(100,Math.round(n/5)*5));allowed.push(`width:${snapped}%`);} }
    else if(key==='font-size') { const m=value.match(/^(\d{1,2})(?:px)?$/i); if(m){const n=Math.max(10,Math.min(64,Number(m[1])));allowed.push(`font-size:${n}px`);} }
    else if(key==='font-weight' && /^(?:bold|[5-9]00)$/i.test(value)) allowed.push('font-weight:700');
    else if(key==='font-style' && lower==='italic') allowed.push('font-style:italic');
    else if(key==='text-decoration') { const parts=[]; if(lower.includes('underline'))parts.push('underline'); if(lower.includes('line-through'))parts.push('line-through'); if(parts.length)allowed.push(`text-decoration:${parts.join(' ')}`); }
    else if(key==='font-family'){
      const normalized=value.replace(/["']/g,'').split(',')[0].trim().toLowerCase();
      const safeMap={'segoe ui':'Segoe UI','arial':'Arial','tahoma':'Tahoma','verdana':'Verdana','trebuchet ms':'Trebuchet MS','calibri':'Calibri','candara':'Candara','corbel':'Corbel','helvetica neue':'Helvetica Neue','helvetica':'Helvetica','roboto':'Roboto','noto sans':'Noto Sans','dejavu sans':'DejaVu Sans','liberation sans':'Liberation Sans','times new roman':'Times New Roman','cambria':'Cambria','palatino linotype':'Palatino Linotype','noto serif':'Noto Serif','dejavu serif':'DejaVu Serif','liberation serif':'Liberation Serif'};
      if(safeMap[normalized])allowed.push(`font-family:${safeMap[normalized]}`);
    }
  }
  return allowed.join(';');
}
function safeNewsHref(value){
  const raw=String(value||'').trim().slice(0,2000); if(!raw)return '';
  if(raw.startsWith('/')&&!raw.startsWith('//'))return raw;
  if(/^(?:https?:|mailto:|tel:)/i.test(raw))return raw;
  return '';
}
function safeNewsImageSrc(value){
  const raw=String(value||'').trim();
  if(raw==='/assets/logo.png')return raw;
  if(raw.startsWith('/uploads/')){
    let rel=''; try{rel=decodeURIComponent(raw.slice('/uploads/'.length));}catch{return '';}
    rel=normalizeUploadPath(rel); if(rel&&rel.startsWith(`${UPLOAD_LAYOUT.news}/`))return uploadUrl(rel)||'';
  }
  const rel=normalizeUploadPath(raw); if(rel&&rel.startsWith(`${UPLOAD_LAYOUT.news}/`))return uploadUrl(rel)||'';
  return '';
}
function sanitizeNewsHtml(value,maxChars=120000){
  let html=String(value||'').replace(/\u0000/g,'').slice(0,maxChars*2);
  html=html.replace(/<!--[^]*?-->/g,'').replace(/<(script|style|iframe|object|embed|form|input|button|textarea|select|option)[^>]*>[^]*?<\/\1\s*>/gi,'');
  const out=[]; let textCount=0; const tokenRe=/<[^>]*>|[^<]+/g; let m;
  while((m=tokenRe.exec(html))){
    const token=m[0];
    if(token[0]!=='<'){
      const cleaned=token.replace(/[\u0000-\u0008\u000b\u000c\u000e-\u001f\u007f]/g,'');
      const remaining=maxChars-textCount; if(remaining<=0)break; const piece=cleaned.slice(0,remaining); textCount+=piece.length; out.push(piece); continue;
    }
    const close=token.match(/^<\s*\/\s*([a-z0-9]+)[^>]*>$/i); if(close){const tag=close[1].toLowerCase();if(NEWS_ALLOWED_TAGS.has(tag)&&!NEWS_VOID_TAGS.has(tag))out.push(`</${tag}>`);continue;}
    const open=token.match(/^<\s*([a-z0-9]+)([^>]*)>$/i); if(!open)continue; const tag=open[1].toLowerCase(); if(!NEWS_ALLOWED_TAGS.has(tag))continue;
    const rawAttrs=open[2]||''; const attrs={}; const attrRe=/([a-zA-Z_:][-a-zA-Z0-9_:.]*)\s*=\s*(?:"([^"]*)"|'([^']*)'|([^\s"'=<>`]+))/g; let a;
    while((a=attrRe.exec(rawAttrs))){attrs[a[1].toLowerCase()]=a[2]??a[3]??a[4]??'';}
    const kept=[];
    const style=normalizeNewsStyle(attrs.style,tag); if(style)kept.push(`style="${escapeHtmlAttr(style)}"`);
    if(tag==='a'){
      const href=safeNewsHref(attrs.href); if(href){kept.push(`href="${escapeHtmlAttr(href)}"`);kept.push('rel="noopener noreferrer"');if(/^https?:/i.test(href))kept.push('target="_blank"');}
    } else if(tag==='th'||tag==='td'){
      const colspan=Math.max(1,Math.min(20,Math.trunc(Number(attrs.colspan)||1))),rowspan=Math.max(1,Math.min(50,Math.trunc(Number(attrs.rowspan)||1)));
      if(colspan>1)kept.push(`colspan="${colspan}"`);if(rowspan>1)kept.push(`rowspan="${rowspan}"`);
    } else if(tag==='img'){
      const src=safeNewsImageSrc(attrs.src); if(!src)continue; kept.push(`src="${escapeHtmlAttr(src)}"`); kept.push(`alt="${escapeHtmlAttr(String(attrs.alt||'').slice(0,240))}"`); kept.push('loading="lazy"');
    }
    if(tag==='figure')kept.push('class="news-article-figure"');
    if(tag==='figcaption')kept.push('class="news-article-caption"');
    if(tag==='table')kept.push('class="rich-content-table"');
    out.push(`<${tag}${kept.length?' '+kept.join(' '):''}>`);
  }
  return out.join('').trim();
}
function sanitizeRichHtml(value,maxChars=8000){
  return sanitizeNewsHtml(value,maxChars).replace(/<img\b[^>]*>/gi,'').replace(/<\/?(?:figure|figcaption)\b[^>]*>/gi,'').trim();
}
function firstNewsImagePath(html){const m=String(html||'').match(/<img\b[^>]*\bsrc=["']([^"']+)["']/i);if(!m)return '';const src=safeNewsImageSrc(m[1]);if(!src||!src.startsWith('/uploads/'))return '';try{return normalizeUploadPath(decodeURIComponent(src.slice('/uploads/'.length)));}catch{return '';}}
...
class Store {
  ...
  initialize() {
    this.db.exec(`
      ...
      CREATE TABLE IF NOT EXISTS contributions (
        ...
        amount_known INTEGER NOT NULL DEFAULT 1 CHECK(amount_known IN (0,1)),
      ...
      CREATE INDEX IF NOT EXISTS idx_news_public_date ON news(is_public, published_date DESC, sort_order DESC, created_at DESC);
      CREATE INDEX IF NOT EXISTS idx_news_title ON news(title COLLATE NOCASE);
    ...
    const contributionColumns = new Set(this.db.prepare('PRAGMA table_info(contributions)').all().map((r)=>r.name));
    if (!contributionColumns.has('address')) this.db.exec("ALTER TABLE contributions ADD COLUMN address TEXT NOT NULL DEFAULT ''");
    if (!contributionColumns.has('amount_known')) this.db.exec("ALTER TABLE contributions ADD COLUMN amount_known INTEGER NOT NULL DEFAULT 1");
    ...
    const defaults = {
      ...
      link_preview_title: 'Cây Gia Phả',
      link_preview_description: 'Cây gia phả gia đình - xem phả hệ, thông tin các thế hệ và trao đổi bình luận.',
    };
  ...
  ensureUploadLayout() {
    fs.mkdirSync(UPLOAD_DIR,{recursive:true});
    for(const name of [UPLOAD_LAYOUT.logo,UPLOAD_LAYOUT.qrcode,UPLOAD_LAYOUT.profiles,UPLOAD_LAYOUT.gallery,UPLOAD_LAYOUT.contacts,UPLOAD_LAYOUT.temple,UPLOAD_LAYOUT.richtext,UPLOAD_LAYOUT.news]) fs.mkdirSync(path.join(UPLOAD_DIR,name),{recursive:true});
  }
  ...
  migrateUploadLayout() {
    ...
    for(const row of this.db.prepare("SELECT id,cover_image_path FROM news WHERE cover_image_path IS NOT NULL AND cover_image_path<>''").all()){const next=migrate(row.cover_image_path,UPLOAD_LAYOUT.news);if(next!==row.cover_image_path)this.db.prepare('UPDATE news SET cover_image_path=? WHERE id=?').run(next,row.id);}
  ...
  normalizeNewsInput(input,current={}) {
    const title=String(input?.title??current.title??'').replace(/[\u0000-\u001f\u007f]/g,' ').replace(/\s+/g,' ').trim().slice(0,220);
    if(!title)throw new Error('Tiêu đề tin tức không được để trống.');
    const slugBase=String(input?.slug??current.slug??title).normalize('NFD').replace(/[\u0300-\u036f]/g,'').toLowerCase().replace(/đ/g,'d').replace(/[^a-z0-9]+/g,'-').replace(/^-+|-+$/g,'').slice(0,180)||`tin-${Date.now()}`;
    let summary=String(input?.summary??current.summary??'').replace(/\r\n?/g,'\n').replace(/[\u0000-\u0008\u000b\u000c\u000e-\u001f\u007f]/g,'').trim().slice(0,700);
    const contentHtml=sanitizeNewsHtml(input?.content_html??current.content_html??'',120000);
    if(!newsPlainText(contentHtml,5))throw new Error('Nội dung tin tức không được để trống.');
    if(!summary)summary=newsPlainText(contentHtml,360);
    const rawCover=input?.cover_image_path===undefined?current.cover_image_path:input.cover_image_path;
    const coverImagePath=rawCover?normalizeUploadPath(rawCover):null;
    if(coverImagePath&&!coverImagePath.startsWith(`${UPLOAD_LAYOUT.news}/`))throw new Error('Ảnh đại diện tin tức không nằm đúng thư mục.');
    const publishedDate=String(input?.published_date??current.published_date??'').trim();
    if(!/^\d{4}-\d{2}-\d{2}$/.test(publishedDate))throw new Error('Ngày đăng tin phải theo định dạng YYYY-MM-DD.');
    const dt=new Date(`${publishedDate}T00:00:00Z`);if(Number.isNaN(dt.getTime())||dt.toISOString().slice(0,10)!==publishedDate)throw new Error('Ngày đăng tin không hợp lệ.');
    const isPublic=input?.is_public==null?(current.is_public==null?1:Number(current.is_public)?1:0):(input.is_public===false||String(input.is_public)==='0'?0:1);
    const sortOrder=input?.sort_order==null?Number(current.sort_order||0):Math.max(-9999,Math.min(9999,Math.trunc(Number(input.sort_order)||0)));
    return {title,slug:slugBase,summary,content_html:contentHtml,cover_image_path:coverImagePath,published_date:publishedDate,is_public:isPublic,sort_order:sortOrder};
  }
  uniqueNewsSlug(base,id='') { let slug=String(base||'tin').slice(0,180),n=2; while(this.db.prepare('SELECT id FROM news WHERE slug=? COLLATE NOCASE AND id<>?').get(slug,id||''))slug=`${String(base||'tin').slice(0,170)}-${n++}`; return slug; }
  presentNews(row,{publicOnly=false,full=true}={}){
    if(!row)return null; const firstImage=firstNewsImagePath(row.content_html); const cover=row.cover_image_path?normalizeUploadPath(row.cover_image_path):''; const image=cover||firstImage;
    return {id:row.id,title:row.title,slug:row.slug,summary:row.summary||newsPlainText(row.content_html,360),content_html:full?row.content_html:undefined,cover_image_url:image?(uploadUrl(image)||'/assets/logo.png'):'/assets/logo.png',published_date:row.published_date,is_public:!!row.is_public,sort_order:Number(row.sort_order||0),created_at:row.created_at,updated_at:row.updated_at,...(!publicOnly?{cover_image_path:cover||null,created_by:row.created_by,updated_by:row.updated_by,created_by_name:row.created_by_name||null,updated_by_name:row.updated_by_name||null}:{})};
  }
  listNews({publicOnly=false,limit=0,full=false}={}){
    const where=publicOnly?'WHERE n.is_public=1 AND n.published_date<=date(\'now\',\'localtime\')':''; const sql=`SELECT n.*,cu.display_name AS created_by_name,uu.display_name AS updated_by_name FROM news n LEFT JOIN users cu ON cu.id=n.created_by LEFT JOIN users uu ON uu.id=n.updated_by ${where} ORDER BY n.published_date DESC,n.sort_order DESC,n.created_at DESC,n.title COLLATE NOCASE${Number(limit)>0?' LIMIT ?':''}`;
    const rows=Number(limit)>0?this.db.prepare(sql).all(Math.max(1,Math.min(100,Math.trunc(Number(limit))))):this.db.prepare(sql).all(); return rows.map(r=>this.presentNews(r,{publicOnly,full}));
  }
  getNews(idOrSlug,{publicOnly=false}={}){const key=String(idOrSlug||'');const where=publicOnly?'AND is_public=1 AND published_date<=date(\'now\',\'localtime\')':'';const row=this.db.prepare(`SELECT * FROM news WHERE (id=? OR slug=? COLLATE NOCASE) ${where} LIMIT 1`).get(key,key);return this.presentNews(row,{publicOnly,full:true});}
  newsYears(){return this.db.prepare("SELECT DISTINCT substr(published_date,1,4) AS year FROM news WHERE published_date GLOB '[0-9][0-9][0-9][0-9]-*' ORDER BY year DESC").all().map(r=>String(r.year)).filter(y=>/^\d{4}$/.test(y));}
  createNews(input,actorId){const x=this.normalizeNewsInput(input),id=uuid(),t=nowIso(),slug=this.uniqueNewsSlug(x.slug);this.db.prepare('INSERT INTO news(id,title,slug,summary,content_html,cover_image_path,published_date,is_public,sort_order,created_by,updated_by,created_at,updated_at) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?)').run(id,x.title,slug,x.summary,x.content_html,x.cover_image_path,x.published_date,x.is_public,x.sort_order,actorId,actorId,t,t);this.audit(actorId,'news.create','news',id,JSON.stringify({title:x.title,date:x.published_date,is_public:!!x.is_public}));return this.getNews(id);}
  updateNews(id,input,actorId){const current=this.db.prepare('SELECT * FROM news WHERE id=?').get(id);if(!current)return null;const x=this.normalizeNewsInput(input,current),slug=this.uniqueNewsSlug(x.slug,id);this.db.prepare('UPDATE news SET title=?,slug=?,summary=?,content_html=?,cover_image_path=?,published_date=?,is_public=?,sort_order=?,updated_by=?,updated_at=? WHERE id=?').run(x.title,slug,x.summary,x.content_html,x.cover_image_path,x.published_date,x.is_public,x.sort_order,actorId,nowIso(),id);this.audit(actorId,'news.update','news',id,JSON.stringify({title:x.title,date:x.published_date,is_public:!!x.is_public}));return this.getNews(id);}
  deleteNews(id,actorId){const current=this.db.prepare('SELECT * FROM news WHERE id=?').get(id);if(!current)return null;this.db.prepare('DELETE FROM news WHERE id=?').run(id);this.audit(actorId,'news.delete','news',id,JSON.stringify({title:current.title}));return this.presentNews(current,{full:true});}
  ...
  normalizeContributionInput(input,current={}) {
    ...
    const address=String(input?.address??current.address??'').replace(/\r\n?/g,'\n').replace(/[\u0000-\u0008\u000b\u000c\u000e-\u001f\u007f]/g,'').trim().slice(0,3000);
    ...
    let amount=Number(current.amount||0),amountKnown=current.amount_known==null?1:(Number(current.amount_known)?1:0);
    if(hasAmountField){
      const raw=input.amount;
      if(raw===null||raw===undefined||String(raw).trim()===''){amount=0;amountKnown=0;}
      else {amount=Math.trunc(Number(raw));amountKnown=1;}
    }
    ...
    return {donor_name:donorName,address,contribution_content:contributionContent,amount,amount_known:amountKnown,contribution_date:contributionDate,notes};
  }
  presentContribution(row){if(!row)return null;return {...row,amount:Number(row.amount||0),amount_known:row.amount_known==null?true:!!row.amount_known};}
  ...
  contributionGroupIdentity(row){
    const norm=(value)=>String(value||'').normalize('NFC').replace(/\s+/gu,' ').trim().toLocaleLowerCase('vi');
    const donor=norm(row?.donor_name),address=norm(row?.address);
    return {donor,address,key:crypto.createHash('sha256').update(`${donor}\u0000${address}`,'utf8').digest('hex').slice(0,32)};
  }
  topContributors(limit=10){
    const n=[5,10,15,20].includes(Number(limit))?Number(limit):10,groups=new Map();
    for(const row of this.listContributions()){
      const identity=this.contributionGroupIdentity(row);
      if(!identity.donor)continue;
      let g=groups.get(identity.key);
      if(!g){g={id:row.id,group_key:identity.key,donor_name:row.donor_name,address:row.address||'',contribution_count:0,known_amount_count:0,amount:0,contribution_date:row.contribution_date||''};groups.set(identity.key,g);}
      g.contribution_count+=1;
      if(row.amount_known){g.known_amount_count+=1;g.amount+=Number(row.amount||0);}
      if(String(row.contribution_date||'')>String(g.contribution_date||'')){g.contribution_date=row.contribution_date;g.id=row.id;g.donor_name=row.donor_name;g.address=row.address||'';}
    }
    return [...groups.values()].filter(g=>g.known_amount_count>0).sort((a,b)=>b.amount-a.amount||String(b.contribution_date||'').localeCompare(String(a.contribution_date||''))||String(a.donor_name||'').localeCompare(String(b.donor_name||''),'vi',{sensitivity:'base'})).slice(0,n).map(g=>({...g,amount_known:true,contribution_content:`Tổng ${g.contribution_count} lần công đức`,notes:`Tổng hợp từ ${g.contribution_count} lần công đức`}));
  }
  contributionGroupItems(groupKey){
    const key=String(groupKey||'').trim().toLowerCase();
    if(!/^[a-f0-9]{32}$/.test(key))return [];
    return this.listContributions().filter(row=>this.contributionGroupIdentity(row).key===key).sort((a,b)=>String(b.contribution_date||'').localeCompare(String(a.contribution_date||''))||String(b.created_at||'').localeCompare(String(a.created_at||'')));
  }
  contributionSummary(rows=null){const list=Array.isArray(rows)?rows:this.listContributions();const donors=new Set(list.map((r)=>this.contributionGroupIdentity(r).key).filter(Boolean));return {count:list.length,donors:donors.size,total_amount:list.reduce((sum,r)=>sum+(r.amount_known?Number(r.amount||0):0),0)};}
  createContribution(input,actorId){const x=this.normalizeContributionInput(input);const id=uuid(),t=nowIso();this.db.prepare('INSERT INTO contributions(id,donor_name,address,contribution_content,amount,amount_known,contribution_date,notes,created_by,updated_by,created_at,updated_at) VALUES (?,?,?,?,?,?,?,?,?,?,?,?)').run(id,x.donor_name,x.address,x.contribution_content,x.amount,x.amount_known,x.contribution_date,x.notes,actorId,actorId,t,t);this.audit(actorId,'contribution.create','contribution',id,JSON.stringify({donor_name:x.donor_name,amount:x.amount_known?x.amount:null,date:x.contribution_date}));return this.getContribution(id);}
  ...
  stats(){ const all=this.rawPeople(); const base=this.treeStats(all); return { ...base, generations:all.length?Math.max(...all.map((p)=>p.level)):0, branches:Number(this.db.prepare('SELECT COUNT(*) AS c FROM branches').get().c||0), comments:Number(this.db.prepare('SELECT COUNT(*) AS c FROM comments WHERE deleted_at IS NULL').get().c||0), news:Number(this.db.prepare('SELECT COUNT(*) AS c FROM news').get().c||0), contributions:Number(this.db.prepare('SELECT COUNT(*) AS c FROM contributions').get().c||0), contribution_total:Number(this.db.prepare('SELECT COALESCE(SUM(amount),0) AS c FROM contributions').get().c||0), users:Number(this.db.prepare('SELECT COUNT(*) AS c FROM users WHERE is_active=1').get().c||0) }; }
  ...
  exportFullBackup(){
    ...
    return {
      ...
      summary:{people:tables.persons.length,branches:tables.branches.length,users:tables.users.length,contributions:tables.contributions.length,news:tables.news.length,comments:tables.comments.length,audit_logs:tables.audit_logs.length,page_visits:tables.page_visits.length,uploads:uploads.length,upload_bytes:uploadBytes},
  ...
  restoreFullBackup(payload, actorId, currentSessionHash=null){
    ...
    const tables={}; let totalRows=0; const legacyWithoutContributions=payload.tables.contributions===undefined; const legacyWithoutNews=payload.tables.news===undefined;
    for(const table of BACKUP_TABLES){
      let rows=payload.tables[table];
      // Bản legacy v3 (trước v1.0.30) chưa có bảng contributions. Khôi phục như danh sách rỗng.
      if((table==='contributions'||table==='news') && rows===undefined) rows=[];
    ...
    if(!integrityOk && (legacyWithoutContributions||legacyWithoutNews)){
      const legacyTables={};
      for(const table of ['settings','users','persons','branches',...(legacyWithoutContributions?[]:['contributions']),...(legacyWithoutNews?[]:['news']),'comments','audit_logs','page_visits']) legacyTables[table]=tables[table];
    ...
    for(const row of tables.news){if(row?.cover_image_path){const n=normalizeUploadPath(row.cover_image_path);if(!n)throw new Error('Đường dẫn ảnh đại diện tin tức trong bản sao lưu không hợp lệ.');requiredUploads.add(n);}}
    ...
    try{
      ...
      this.db.exec('DELETE FROM branches; DELETE FROM contributions; DELETE FROM news; DELETE FROM comments; DELETE FROM audit_logs; DELETE FROM page_visits; DELETE FROM persons; DELETE FROM users; DELETE FROM settings;');
      insertRows('settings',tables.settings); insertRows('users',tables.users); insertRows('persons',tables.persons); insertRows('branches',tables.branches); insertRows('contributions',tables.contributions); insertRows('news',tables.news); insertRows('comments',tables.comments); insertRows('audit_logs',tables.audit_logs); insertRows('page_visits',tables.page_visits);
    ...
    return {ok:true,session_preserved:sessionPreserved,exported_at:payload.exported_at||null,summary:{people:tables.persons.length,branches:tables.branches.length,users:tables.users.length,contributions:tables.contributions.length,news:tables.news.length,comments:tables.comments.length,uploads:decoded.length,page_visits:tables.page_visits.length}};
  }
