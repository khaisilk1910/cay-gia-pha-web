...
function normalizeNewsStyle(style,tag){
  ...
  for(const chunk of String(style||'').split(';')){
    ...
    else if(key==='background-color' && ['th','td'].includes(tag) && (/^#[0-9a-f]{6}$/i.test(value)||/^rgb\(\s*\d{1,3}\s*,\s*\d{1,3}\s*,\s*\d{1,3}\s*\)$/i.test(value))) allowed.push(`background-color:${value}`);
