...
function renderBrandAndFooter(){window.GiaPhaPublicUI?.applyBranding?.(state.settings);window.GiaPhaPublicUI?.initWelcomePopup?.(state.settings);window.GiaPhaPublicUI?.renderLatestNews?.(document);const logo=$('#brandLogo');if(logo){logo.onerror=()=>{logo.onerror=null;logo.src='/assets/logo.png';};logo.src=state.settings.logo_url||'/assets/logo.png';}renderFundSupport();const clan=state.settings.clan_name||'Gia đình';renderRichText($('#treeFooterContent'),state.settings.tree_footer_content,`${clan} · Dữ liệu gia đình được trình bày với ưu tiên quyền riêng tư.`,{size:14,align:'center'});const author=$('#footerAuthor');if(author){const has=renderRichText(author,state.settings.footer_author_content,state.settings.footer_author_text||'',{size:14,align:'center'});author.classList.toggle('hidden',!has);}}
