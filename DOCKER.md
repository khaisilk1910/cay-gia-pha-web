# Chạy Cây Gia Phả Web v1.0.33 bằng Docker

Bản Docker v1.0.33 bao gồm đầy đủ tính năng web v1.0.33 và tiếp tục lưu rõ mã ứng dụng + dữ liệu trên Ubuntu:
...
Không cần `${IMAGE_TAG}`. Mỗi lần redeploy Stack, Docker Compose sẽ kiểm tra/pull image `latest`. Nếu image mới có nội dung khác, entrypoint của v1.0.33 tự cập nhật `/opt/cay-gia-pha-web/app/`; thư mục `/opt/cay-gia-pha-web/data/` không bị ghi đè.

> Khuyến nghị: trước mỗi lần cập nhật `latest`, nên tạo `.gpbak` từ giao diện quản trị **và sao lưu riêng `data/uploads/gallery/`**. `latest` tiện, nhưng tag cố định như `1.0.33` vẫn dễ rollback hơn.
...
2. Tạo tag mới, ví dụ `v1.0.33`.
3. GitHub Actions build/push cả `:1.0.33` và `:latest`.
...
File `.github/workflows/docker-publish.yml` đã có trong bản Docker-ready. Khi push tag `v1.0.33`, workflow tạo:

```text
ghcr.io/khaisilk1910/cay-gia-pha-web:1.0.33
...
```bash
docker build --pull -t cay-gia-pha-web:1.0.33 .
```
...
Với Stack v1.0.33, **mã ứng dụng Cây Gia Phả Web và toàn bộ dữ liệu của ứng dụng** đều nằm rõ ràng trong `/opt/cay-gia-pha-web/`.
